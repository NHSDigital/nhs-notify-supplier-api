...
const getMockedDeps = (): jest.Mocked<Deps> => {
  return {
...
describe("API Lambda handler", () => {
  it("passes if S3 and DynamoDB are available", async () => {
    const event = makeApiGwEvent({ path: "/_status", headers: undefined });

    const getLetterDataHandler = createGetStatusHandler(getMockedDeps());
    const result = await getLetterDataHandler(
      event,
      mockDeep<Context>(),
      jest.fn(),
    );
  ...
  it("fails if DynamoDB is unavailable", async () => {
    const mockedDeps = getMockedDeps();
    mockedDeps.dbHealthcheck.check = jest
      .fn()
      .mockRejectedValue(new Error("unexpected error"));
