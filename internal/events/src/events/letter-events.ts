 ...
 */
export const $LetterEvent = $EnvelopeProfile
  .safeExtend({
    type: z
      .enum(
        $LetterStatus.options.map(
          (status) => `uk.nhs.notify.supplier-api.letter.${status}.v1`,
        ),
      )
      .meta({
    ...
    dataschema: z
      ...
      .regex(
        new RegExp(
          `^https:\\/\\/notify\\.nhs\\.uk\\/cloudevents\\/schemas\\/supplier-api\\/letter\\.(?<status>${$LetterStatus.options.join("|")})\\.1\\.\\d+\\.\\d+\\.schema.json$`,
        ),
      )
      .meta({
...
Data schema version must match the major version indicated by the type`,
    ...
    source: z
      ...
      .meta({
        title: "Event Source",
        description:
          "Logical event producer path within the supplier-api domain",
      }),

    subject: z
      .string()
      .regex(/^letter-origin\/[a-z0-9-]+\/letter\/[^/]+(?:\/.*)?/)
      .meta({
        title: "Event Subject",
        description:
  ...
  $LetterEvent
    .safeExtend({
      ...
      dataschema: z
        ...
        .regex(
          new RegExp(
            `^https:\\/\\/notify\\.nhs\\.uk\\/cloudevents\\/schemas\\/supplier-api\\/letter.${status}.1\\.\\d+\\.\\d+\\.schema.json$`,
          ),
        )
        .meta({
          title: "Data Schema URI",
